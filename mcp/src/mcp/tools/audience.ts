...
export function registerAudienceTools(server: McpServer): void {
  addTool<{
    ...
    server,
    'preview_audience',
    'Preview how many opted-in guests a campaign segment reaches, broken down per channel (email needs an address + no unsubscribe; SMS a phone + no STOP; WhatsApp a phone + no opt-out). Always show these numbers to the user before sending.',
    {
    ...
    async (args, extra) => {
      ...
      return jsonResult({ total, perChannel });
    },
  );
}
